...
import { getMTierList, FIGHTER_COUNT } from '../../src/models/m-tier-list';
...
describe('MTierList - Stat Tracking', () => {
  describe('adjustTierSlotPositionBySteps with trackStats', () => {
    it('should increment contestCount and winCount when trackStats is true (default)', () => {
      const slots = [
        createMockTierSlot({ id: 'slot-1', position: 5, contestCount: 5, winCount: 2 }),
        createMockTierSlot({ id: 'slot-2', position: 6, contestCount: 3, winCount: 1 })
      ];
...
describe('MTierList - Diff Checking', () => {
  describe('getChangedTierSlots', () => {
    it('should return only slots with changed positions', () => {
      const slots = [
        createMockTierSlot({ id: 'slot-1', position: 0, contestCount: 5, winCount: 2 }),
        createMockTierSlot({ id: 'slot-2', position: 1, contestCount: 3, winCount: 1 }),
        createMockTierSlot({ id: 'slot-3', position: 2, contestCount: 8, winCount: 4 })
      ];
      const mockTierList = createMockTierList(slots);
      const mTierList = getMTierList(mockTierList);
    ...
    it('should handle null positions correctly', () => {
      const slots = [
        createMockTierSlot({ id: 'slot-1', position: null as any, contestCount: 0, winCount: 0 }),
        createMockTierSlot({ id: 'slot-2', position: 1, contestCount: 3, winCount: 1 })
      ];
      const mockTierList = createMockTierList(slots);
      const mTierList = getMTierList(mockTierList);
  ...
  describe('getPositionsPojo', () => {
    it('should return all slots with stats using tierSlotN keys', () => {
      const slots = [
        createMockTierSlot({ id: 'slot-1', position: 0, contestCount: 5, winCount: 2 }),
        createMockTierSlot({ id: 'slot-2', position: 1, contestCount: 3, winCount: 1 }),
        createMockTierSlot({ id: 'slot-3', position: null as any, contestCount: 0, winCount: 0 })
      ];
      const mockTierList = createMockTierList(slots);
      const mTierList = getMTierList(mockTierList);
...
describe('MTierList - Position Validation', () => {
  ...
  it('should clamp positions to valid range in sparse mode', () => {
    ...
    const slots = [
      createMockTierSlot({ id: 'slot-1', position: 5, contestCount: 0, winCount: 0 }),
      createMockTierSlot({ id: 'slot-2', position: null as any, contestCount: 0, winCount: 0 })
    ];
    const mockTierList = createMockTierList(slots);
    const mTierList = getMTierList(mockTierList);
  ...
  it('should log error for out-of-bounds position in fully positioned mode', () => {
    ...
    const slots = Array.from({ length: 100 }, (_, i) =>
      createMockTierSlot({ id: `slot-${i}`, position: i, contestCount: 0, winCount: 0 })
    );
    const mockTierList = createMockTierList(slots);
    const mTierList = getMTierList(mockTierList);
    ...
    const hasInvalidPositionError = errorCalls.some((call) =>
      call.some((arg) => typeof arg === 'string' && arg.includes('INVALID POSITION'))
    );
  ...
  it('should handle boundary positions safely', () => {
    ...
    const slots = [
      createMockTierSlot({ id: 'slot-1', position: 84, contestCount: 0, winCount: 0 }),
      createMockTierSlot({ id: 'slot-2', position: null as any, contestCount: 0, winCount: 0 })
    ];
    const mockTierList = createMockTierList(slots);
    const mTierList = getMTierList(mockTierList);
...
describe('MTierList - Integration Tests', () => {
  ...
  it('should detect only actually changed slots after position adjustments', () => {
    const slots = Array.from({ length: 25 }, (_, i) =>
      createMockTierSlot({ id: `slot-${i}`, position: i, contestCount: i, winCount: 0 })
    );
    const mockTierList = createMockTierList(slots);
    const mTierList = getMTierList(mockTierList);
