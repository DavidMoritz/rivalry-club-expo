...
function TierSlotStats({ tierSlot }: { tierSlot: MTierSlot }) {
  ...
  const canShowWinRate =
    hasContestCount && hasWinCount && (tierSlot.contestCount ?? 0) > 0;
  ...
  return (
    <>
      ...
      {hasPosition &&
        ...
        tierSlot.position !== undefined && (
          <Text style={statTextStyle}>
            Position: #{tierSlot.position + 1} (Tier{' '}
            {computeTierFromPosition(tierSlot.position ?? 0)})
          </Text>
        )}
      {hasContestCount && (
        <Text style={statTextStyle}>
      ...
      {canShowWinRate &&
        ...
        tierSlot.contestCount !== undefined && (
          <Text style={winRateTextStyle}>
            Rivalry Win Rate:{' '}
            {calculateWinRate(tierSlot.winCount, tierSlot.contestCount)}%
          </Text>
        )}
    </>
  );
...
function FighterStats({
  ...
  const canShowWinRate =
    hasContestCount && hasWinCount && (fighter.contestCount || 0) > 0;
