...
export function getMTierList(tierList: TierList): MTierList {
  // constructors
  const _mTierSlots = ((tierList.tierSlots?.items || []) as TierSlot[]).map(
    ts => getMTierSlot(ts),
  );
  ...
  return {
    ...
    sampleEligibleSlot() {
      if (!this.rivalry) return sample(this.slots) as MTierSlot;
      ...
      const avoidSlotIds: string[] = this.rivalry?.mContests
        .slice(0, benchedRounds)
        .map(c => (isA ? c.tierSlotAId : c.tierSlotBId));
      let selectSlots;

      // Keep trying with fewer and fewer benched rounds
      while (benchedRounds > 0) {
        selectSlots = eligibleSlots.filter(
          ts => !avoidSlotIds.slice(0, benchedRounds).includes(ts.id),
        );
